...
async function serveToolLogo(stateRoot, id) {
  const paths = await readStatePaths(resolveStateRoot(stateRoot))
  const state = await loadState(paths)
...
async function action(value, stateRoot, dependencies = {}) {
  ...
  if (value.action === 'setup') {
    ...
    return setup({
      ...
      hosts: value.host === undefined ? [] : [value.host],
      noHost: value.host === undefined,
  ...
  if (value.action === 'updates-install') {
    return updatesInstall({
      stateRoot,
      id: value.id,
      all: value.all === true,
      includeApp: value.includeApp === true,
      github: value.github,
    })
  }
  ...
  if (value.action === 'repair') {
    return repairInstallation({ stateRoot, dryRun: false })
  }
...
"All tools":"全部工具",
"Tool":"工具",
"Installed {version}":"当前安装 {version}",
"Show more tools":"展开更多工具",
"Show less":"收起",
"Agent task":"Agent 任务",
"Health check":"健康检查",
"Validation":"验证",
"Purpose not recorded":"未记录用途",
"Result details unavailable":"未记录结果详情",
"Copy analysis request":"复制分析请求",
"Analysis request copied":"已复制分析请求",
"{count} observations · {current} since this binding":"{count} 次观测 · 当前绑定期间 {current} 次",
"{count} from script references":"其中 {count} 次来自脚本引用",
"{count} executions · {errors} runtime errors":"{count} 次执行 · {errors} 次运行错误",
"{count} partial results · {items} failed items":"{count} 次部分失败 · {items} 个失败项",
"Showing {shown} of {available} groups":"显示 {shown} / {available} 组",
...
function availabilityLabel(value){
  return t({
    ...
    'no-platform-asset':'No asset for this platform',
    'check-failed':'check failed',
    'compatible-after-host-update':'compatible after Host update',
    'installed-official-upgrade':'official upgrade',
    'installed-version-unreadable':'installed, version unread'
  }[value]||value||'—')
}
function renderUpdates(){
  ...
  if(!items.length) versions.append(el('p',t('Check for updates to load current and available versions.'),'muted'));
  for(const item of items){
    const r=el('div',undefined,'row');
    const label=el('div',undefined,'grow');
    const title=el('div');title.append(logoNode(item.logo,item.id),document.createTextNode(' '+(item.displayName||item.id)));
    label.append(title,el('div',[item.installedVersion||t('not installed'),item.availableVersion].filter(Boolean).join(' → ')+' · '+availabilityLabel(item.availability),'muted'));
    r.append(label);
    if(item.kind==='tool'&&item.availability==='update-available') r.append(button('Install update',()=>call({action:'updates-install',id:item.id},t('Adding GitHub tool…')),'action primary'));
    if(item.kind==='application'&&item.availability==='update-available') r.append(button('Install update',()=>call({action:'updates-install',id:'agent-host',includeApp:true},t('Checking updates…')),'action primary'));
    versions.append(r);
  }
  const updatable=items.filter(i=>i.availability==='update-available'&&i.kind==='tool');
...
const featuredExperiences=[
  {id:'math-anchor',name:'Math Anchor',summary:'Exact and scientific calculation',task:'Verify exact math without approximation',outcome:'Exact result with the calculation checked',prompt:'Calculate 9,999,999,999 × 87 exactly, show the result, and verify it.',visual:'9,999,999,999\\n× 87',symbol:'ƒ',tone:'indigo'},
  {id:'migratory-time',name:'Migratory Time',summary:'Reliable worldwide time conversion',task:'Coordinate a time across cities',outcome:'Local date and time with zone rules applied',prompt:'Convert 9:00 AM on October 15, 2026 from Shanghai to San Francisco and state the local date.',visual:'09:00  SHA → SFO',symbol:'◷',tone:'teal'},
  {id:'armorial',name:'Armorial',summary:'Choose project-aware icons without redrawing them',task:'Choose an icon that fits the project',outcome:'A reusable SVG from the project-aware set',prompt:'Choose and render one project-aware icon for a primary Start action. Explain the visual fit briefly.',visual:'◇  ○  ✦',symbol:'◇',tone:'orange'},
];
function experiencesForProfile(profile){return profile==='featured'?featuredExperiences:profile==='standard'||profile==='observability'?featuredExperiences.filter(x=>x.id!=='armorial'):[]}
function capabilityCard(experience,state,onTry){
  const article=el('article',undefined,'capability-card');
  const visual=el('div',undefined,'capability-visual '+experience.tone);
  visual.setAttribute('aria-hidden','true');
  visual.append(el('span',experience.symbol,'symbol'),el('span',experience.visual,'value'));
  const content=el('div');
  content.append(el('div',t(experience.name),'capability-name'),el('div',t(experience.task),'capability-task'),el('div',t(experience.outcome),'muted'),el('div',f('Try: {task}',{task:t(experience.prompt)}),'capability-prompt'));
  const controls=el('div',undefined,'capability-state');
  if(state)controls.append(el('span',t(state)));
  if(onTry){const tryButton=button('Try in a new task',()=>onTry(experience,tryButton),'action primary');controls.append(tryButton)}
  article.append(visual,content,controls);return article;
}
async function copyCapabilityTask(experience,trigger){
  ...
  trigger.textContent=t('Task copied');
  ...
  notice(t(hostIds.length>1?'Task copied. Choose an Agent app to continue.':'Task copied. Connect an Agent app to continue.'));setPage('environment');
}
function renderSetup(root){
  ...
  const renderProfileTools=()=>{
    ...
    const experiences=experiencesForProfile(profile.value);
    if(experiences.length){const intro=el('p',t('Choose by the work you want to do.'),'muted capability-intro'),list=el('div',undefined,'capability-list');for(const experience of experiences)list.append(capabilityCard(experience,null,null));tools.append(intro,list);return}
...
function navigatePage(page){
  ...
  setPage(page);return false;
}
function renderPostSetupGuidance(root, guidance){
  if(!guidance)return;
  const tone=guidance.statusTone||(guidance.readyToWork?'ready':'action');
  const c=el('div',undefined,'card start-work');
  c.dataset.tone=tone;
  const row=el('div',undefined,'start-row');
  const status=el('div',t(guidance.statusLine||guidance.title||'Ready'),'start-status');
  ...
  const runPrimary=()=>{
    ...
    if(action.id==='run-setup'){navigatePage('environment');return}
    notice(t(action.label||'Ready'));
  };
  if(action.label){
    const b=button(action.label,runPrimary,'action primary');
    b.dataset.testid='post-setup-primary-cta';
    b.dataset.actionId=action.id||'';
    row.append(b);
  }
  c.append(row);
  if(guidance.hint&&(action.id==='open-app'||action.id==='start-new-agent-task')){
    c.append(el('p',t(guidance.hint),'muted start-hint'));
  }
...
function renderEnvironment(s,u){
  ...
  moreBody.append(row(t('Application'),(app.version||'—')+(app.build?' ('+app.build+')':'')));
  moreBody.append(row(t('Environment release'),(env.suiteVersion||s.environment?.suiteVersion||'—')+(env.profile?' · '+env.profile:'')));
...
function renderTools(value){
  ...
  const catalog=card('Start from a task');
  const experienceList=el('div',undefined,'capability-list');
  const modeled=new Set();
  for(const experience of featuredExperiences.filter(item=>admitted.includes(item.id))){
    modeled.add(experience.id);
    const isInstalled=installed.has(experience.id);
    const state=isInstalled?(value.paused===true?'Tools paused':'Installed'):'Not installed in this environment';
    experienceList.append(capabilityCard(experience,state,isInstalled&&value.paused!==true?copyCapabilityTask:null));
  }
  catalog.append(experienceList);
  for(const id of admitted.filter(item=>!modeled.has(item))){
  ...
  else c.append(el('p',t('Off keeps an on-demand Skill. Pause all withholds both.'),'muted'));
  ...
  for(const id of available){const label=el('label',undefined,'check'),box=document.createElement('input');box.type='checkbox';box.value=id;box.checked=value.paused!==true&&(value.activeAgentComponents||[]).includes(id);box.onchange=update;const name=el('span',byId[id]?.displayName||value.components?.[id]?.displayName||id);label.append(box,name);if(value.paused===true)label.append(el('span',t('Paused'),'pill warn'));else if(!(value.activeAgentComponents||[]).includes(id))label.append(el('span',t('On-demand'),'pill quiet'));form.append(label)}
  const actions=el('div',undefined,'actions');
...
function renderToolHistory(u,root){
  ...
  const renderTools=()=>{toolList.replaceChildren();const displayed=u.tools.entries.slice(0,expanded?u.tools.entries.length:8);toolCount.textContent=f('Showing {shown} of {available} groups',{shown:displayed.length,available:u.tools.available});for(const item of displayed){
    ...
    const detail=f('{count} observations · {current} since this binding',{count:number(item.historicalCalls),current:number(item.currentBindingCalls)});
    toolList.append(row(title+(item.componentVersion?' · '+f('Installed {version}',{version:item.componentVersion}):''),detail));
    if(item.referencedCalls>0)toolList.append(el('p',f('{count} from script references',{count:number(item.referencedCalls)}),'muted'));
  }};renderTools();tools.append(toolList);
  ...
  for(const id of ['all',...new Set(entries.map(x=>x.providerId))]){const option=el('option',id==='all'?t('All tools'):providerName(id));option.value=id;select.append(option)}
  const render=()=>{list.replaceChildren();for(const item of entries.filter(x=>select.value==='all'||x.providerId===select.value)){
    const purpose=t({task:'Agent task',diagnostic:'Health check',validation:'Validation'}[item.purpose]||'Purpose not recorded');
    const label=providerName(item.providerId)+' · '+(item.providerVersion||'—')+' · '+purpose;
    list.append(row(label,f('{count} executions · {errors} runtime errors',{count:number(item.executions),errors:number((item.providerErrors||0)+(item.hostErrors||0))})));
    if(item.partialResults>0)list.append(el('p',f('{count} partial results · {items} failed items',{count:number(item.partialResults),items:number(item.itemErrors)}),'notice'));
    if(item.reportedOutcomes===0)list.append(el('p',t('Result details unavailable'),'muted'));
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/tool-logo') {
        ...
        if (typeof id !== 'string' || !/^[a-z][a-z0-9-]*$/u.test(id)) {
          json(response, 400, { status: 'error', error: { code: 'MANAGER_REQUEST_INVALID', message: 'Choose one installed tool' } })
          return
        }
        const served = await serveToolLogo(stateRoot, id)
        if (served === null) {
          json(response, 404, { status: 'error', error: { code: 'TOOL_LOGO_UNAVAILABLE', message: 'No logo is available for this tool.' } })
          return
        }
        response.writeHead(200, {
