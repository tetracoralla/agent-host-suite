...
| Active tools and projections | Installed, active, configured and healthy remain distinct. Host working-set `active` is not Agent-app cache verification or current-session discovery. Catalog conflicts and declared resource-limit failures stop activation before mutation; thin projections retain exact provider identity and invocation. | [Profile truth](../test/REVIEW_CASES.md#profile-and-tool-truth), [projections](../test/REVIEW_CASES.md#thin-host-projection), [catalogs](../test/REVIEW_CASES.md#skill-link-catalog-truth), [discovery](DISCOVERY_PROJECTION.md) |
...
| Manager experience | Setup, status, task invitations, tool changes and recovery are understandable and usable. Optional examples remain editable offers rather than scores or requirements. Startup and ordinary refresh avoid intrusive Agent-app inspection; explicit Full Check owns that route. | [Inspection effects](../test/REVIEW_CASES.md#default-inspection-effect-budget), [human flows](../test/REVIEW_CASES.md#human-runtime-flow) |
...
| Provider and task claims | Local process execution does not prove absent network egress or valid credentials. Installation, live health, tool selection and task success are separate observations. A fresh-session page task can inform a participant's decision; Host status cannot replace it or issue the verdict. | [Instance truth](../test/REVIEW_CASES.md#provider-instance-truth), [HTTP bridge](../packages/capability-http-bridge/docs/REVIEW_CONTRACT.md), [catalog analysis](../packages/context-surface-analyzer/docs/REVIEW_CONTRACT.md), [unnamed capability exercise](ADOPTION_ACCEPTANCE.md) |
