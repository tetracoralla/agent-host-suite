...
test('deep doctor reports per-tool direct checks while the service is not running', async (t) => {
  ...
  await setup({ profile: 'standard', hosts: ['codex'], developmentRoot: root, stateRoot, noService: true, dryRun: false, enableObservability: false }, { runner: fake.runner, codexConfiguration: fake.configuration, hostSkillHome: join(stateRoot, 'host-home'), catalogPreflight: healthyCatalogPreflight, applicationStatePreflight: compatibleApplicationState })
  ...
  const shallow = await doctor(state, { codexConfiguration: fake.configuration, runner: serviceDownRunner(fake.runner) })
...
test('doctor warns when the active catalog exceeds its declared resource limit', async () => {
  const state = {
    channel: 'development',
    components: {},
    hosts: {},
  ...
  assert.match(catalog.message, /exceeds 1 declared resource limit/u)
