...
try {
  ...
  const installedWrapModule = join(installRoot, 'node_modules', '@openadam', 'agent-host-suite', 'src', 'github-plugin-wrap.mjs')
  await execFileAsync(process.execPath, ['--input-type=module', '-e', `const cli = await import(${JSON.stringify(pathToFileURL(installedCliModule).href)}); const service = await import(${JSON.stringify(pathToFileURL(installedServiceModule).href)}); const recovery = await import(${JSON.stringify(pathToFileURL(installedRecoveryModule).href)}); const wrap = await import(${JSON.stringify(pathToFileURL(installedWrapModule).href)}); if (typeof cli.main !== 'function' || typeof service.restoreServiceRecoveryBundle !== 'function' || typeof service.retainedLaunchAgentProgram !== 'function' || typeof recovery.rebindServiceRecoveryPartialRestore !== 'function' || typeof wrap.wrapGitHubPluginArchive !== 'function') process.exit(2); if (service.retainedLaunchAgentProgram(Buffer.from('<plist version="1.0"><dict><key>Program</key><string>/opt/node</string></dict></plist>')) !== '/opt/node') process.exit(3); try { service.retainedLaunchAgentProgram(Buffer.from('<plist version="1.0"><dict><key>ProgramArguments</key><array><dict><key>Program</key><string>/opt/nested</string></dict></array></dict></plist>')); process.exit(4) } catch (error) { if (error.code !== 'SERVICE_RECOVERY_BUNDLE_INVALID') process.exit(5) }`])
