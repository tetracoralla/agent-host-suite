...
"Use the installed Agent Host operations skill to analyze the current usage report, version history, runtime errors and coverage. Separate tasks from diagnostics and script references from observed execution. Compare findings with the current task before proposing changes; counts alone do not establish adoption or correctness.":"请使用已安装的 Agent Host operations 技能分析当前使用报告、版本历史、运行错误和采集覆盖。区分真实任务与健康检查、脚本引用与实际执行。结合当前任务提出改进，不要仅凭次数推断采纳或正确性。",
  ...
  'Agent environment':'Agent 环境','Installed locally on this PC':'已安装在这台电脑上','Set up a compatible local tool environment':'设置兼容的本地工具环境','Set up tools':'设置工具','Standard tools':'标准工具','Featured tools':'精选工具','Developer Kit':'开发者 Kit','Standard + monitoring':'标准工具 + 监控','Set up':'设置','Setting up tools…':'正在设置工具…','Check again':'重新检测','Connect later':'稍后连接','Detected on this PC':'已在这台电脑上检测到','Math Anchor':'Math Anchor','Migratory Time':'Migratory Time','Armorial':'Armorial','Exact and scientific calculation':'精确与科学计算','Reliable worldwide time conversion':'可靠的全球时区转换','Choose project-aware icons without redrawing them':'按项目选用图标，无需重绘','Skill-only kit; this profile adds no Agent MCP tools':'仅 Skill；此配置不添加 Agent MCP 工具','Choose a tool set, then install Agent Host. You can connect a supported Agent app now or later.':'先选择工具集并安装 Agent Host；受支持的 Agent 应用可以现在连接，也可以稍后连接。','Featured installs catalog inventory, including Armorial, from a bound compatibility release. It is not a marketplace.':'精选配置会从已绑定的兼容版本安装目录库存（含 Armorial）。这不是应用市场。','This checkout has no public GitHub Release. Setup needs a bound catalog unless a packaged carrier already includes one.':'此源码 checkout 没有公开 GitHub Release。除非安装包已带绑定目录，否则设置需要一份绑定目录。','Connecting an Agent app is optional at install time. If none is detected, install Agent Host now and connect later from Agent apps.':'安装时可以不连接 Agent 应用。若未检测到受支持应用，可先安装 Agent Host，稍后再从“Agent 应用”连接。','No public GitHub Release is published from this checkout. A bound catalog path may be set with AGENT_HOST_RELEASE_MANIFEST. Unsigned macOS builds are not Apple-notarized; Control-click the app, choose Open, then confirm Gatekeeper.':'此 checkout 未发布公开 GitHub Release。可用 AGENT_HOST_RELEASE_MANIFEST 指定绑定目录。未签名的 macOS 安装包未经 Apple 公证：按住 Control 点击应用，选择“打开”，再确认 Gatekeeper。','Featured catalog download':'精选目录下载','Unsigned macOS builds are not Apple-notarized. Control-click the app, choose Open, then confirm the Gatekeeper warning. This is expected until a Developer ID signed build exists.':'未签名的 macOS 安装包未经 Apple 公证。请按住 Control 点击应用，选择“打开”，再确认 Gatekeeper 提示。在提供 Developer ID 签名版本之前，这是预期步骤。',
  ...
  'Choose which installed tools are available to fresh Agent tasks.':'选择新 Agent 任务可以使用哪些已安装工具。','Working set for new tasks':'新任务的工作集','Get featured tools':'获取精选工具','Getting featured tools…':'正在获取精选工具…','Installed':'已安装','Not installed in this environment':'此环境尚未安装','Get installs featured catalog inventory. Checkboxes only enable the working set of tools already installed. tools set --profile does not install missing tools.':'“获取”会安装精选目录库存。复选框只启用已经安装的工具工作集。tools set --profile 不会安装尚未入库的工具。','Owner-selected tools. Not a marketplace, store, ranking, or payment catalog.':'所有者精选的工具；不是应用市场、商店、排行或付费目录。','No Agent environment is installed.':'尚未安装 Agent 环境。','Available tools':'可用工具','Changes take effect in a fresh Agent task.':'更改会在新的 Agent 任务中生效。','Apply tool set':'应用工具集',
...
function renderSetup(root){
  ...
  c.append(el('p',t('Choose a tool set, then install Agent Host. You can connect a supported Agent app now or later.'),'muted'));
  ...
  const renderProfileTools=()=>{
    ...
    tools.append(el('p',t(profile.value==='featured'?'Featured installs catalog inventory, including Armorial, from a bound compatibility release. It is not a marketplace.':'This checkout has no public GitHub Release. Setup needs a bound catalog unless a packaged carrier already includes one.'),'muted'));
  };
  ...
  c.append(tools,a,el('p',t('Connecting an Agent app is optional at install time. If none is detected, install Agent Host now and connect later from Agent apps.'),'muted'));
...
function renderTools(value){
  ...
  catalog.append(el('p',t('Get installs featured catalog inventory. Checkboxes only enable the working set of tools already installed. tools set --profile does not install missing tools.'),'muted'));
  catalog.append(el('p',t('Owner-selected tools. Not a marketplace, store, ranking, or payment catalog.'),'muted'));
  for(const id of admitted){
    const name=value.components?.[id]?.displayName||featuredToolName(id);
  ...
  if(download?.configured&&download.url){
    ...
    catalog.append(el('p',t(download.gatekeeperNote||'Unsigned macOS builds are not Apple-notarized. Control-click the app, choose Open, then confirm the Gatekeeper warning. This is expected until a Developer ID signed build exists.'),'muted'));
  }
  root.append(catalog);
