...
test('doctor explicitly labels new-runtime checks and preserves the legacy closed order', () => {
  assert.equal(createDiagnosticOrder([], '0.2.2').purpose, 'diagnostic')
  assert.equal(createDiagnosticOrder([], '0.2.1').schemaVersion, 'openadam.direct-work-order.v0.1')
  assert.equal(Object.hasOwn(createDiagnosticOrder([], '0.2.1'), 'purpose'), false)
})
