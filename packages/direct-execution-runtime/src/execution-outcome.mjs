import { jsonBytes, snapshotJsonValue } from './json.mjs'
...
export function validExecutionOutcome(value) {
  ...
  if (jsonBytes(value) > 16_384 || !Array.isArray(value.errorCodes) || value.errorCodes.length > 64) return false
  const codes = new Set()
...
// Only the provider's explicit, closed metadata declares an outcome. Arbitrary
// result fields and MCP transport completion cannot supply this observation.
export function readExecutionOutcome(response) {
  const value = response._meta?.[EXECUTION_OUTCOME_META_KEY]
  if (value === undefined) return { status: 'not-reported', value: null }
  if (!validExecutionOutcome(value) || (response.isError === true && !['error', 'cancelled'].includes(value.status))) {
    return { status: 'invalid', value: null }
  }
  return { status: 'reported', value: snapshotJsonValue(value) }
}
