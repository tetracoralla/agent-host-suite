...
test('release setup, update, rollback, and purge retain immutable versions without source roots', async (t) => {
  ...
  await setup({ profile: 'standard', hosts: ['codex'], releaseManifest: firstManifest, stateRoot, noService: true, dryRun: false, enableObservability: false }, { runner: fake.runner })
  ...
  assert.equal((await loadState(paths)).suiteVersion, '0.1.0-beta.2')
...
test('development installation migrates to a release channel without losing enabled local monitoring', async (t) => {
  ...
  await setup({ profile: 'standard', hosts: [], developmentRoot: workspace, stateRoot, noService: true, dryRun: false, enableObservability: false }, { runner: fake.runner })
  const paths = await prepareStatePaths(stateRoot)
  ...
  assert.equal(dryRun.activation.hosts.codex.entries.every((entry) => entry.marketplaceRoot.startsWith(paths.packages)), true)
...
test('an update cannot select a consent-bearing profile before local monitoring is enabled', async (t) => {
  ...
  await setup({ profile: 'standard', hosts: [], releaseManifest: manifest, stateRoot, noService: true, dryRun: false, enableObservability: false }, { runner: fake.runner })
...
test('development to release migration restores the development environment when monitoring rebind fails', async (t) => {
  ...
  const stateRoot = join(root, 'private', 'state', 'root')
  const fake = createCodexRunner({ mathPresent: false, timePresent: false, mathVersion: '0.3.0' })
  await setup({ profile: 'standard', hosts: ['codex'], developmentRoot: workspace, stateRoot, noService: true, dryRun: false, enableObservability: false }, { runner: fake.runner })
  const paths = await prepareStatePaths(stateRoot)
  const development = await loadState(paths)
  development.profile = 'observability'
  development.components['agent-tool-observer'] = { version: '0.1.0', root: workspace, command: process.execPath, args: [], identityFiles: [], fingerprint: 'development-observer' }
  ...
  assert.equal(fake.marketplaces.get('math-anchor'), join(resolvedWorkspace, 'calculator'))
  assert.equal(fake.marketplaces.get('migratory-time'), join(resolvedWorkspace, 'migratory-time'))
