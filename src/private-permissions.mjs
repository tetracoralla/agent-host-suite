...
// Under high process stampede the PowerShell helper can throw or return
// status=error without meaning the directory is shared or owned by someone
// else. Retry only that unavailable class; wrong-owner and permissions-unsafe
// stay fail-closed immediately. Helper timeout retries remain in
// windows-private-access.mjs.
const ACL_VERIFY_ATTEMPTS = 3

function delay(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms))
}

function aclVerifyBackoffMs(attempt) {
  return 50 * (attempt + 1)
}
...
async function windowsAccess(path, ensure) {
  ...
  const operation = (async () => {
    let lastUnavailable
    for (let attempt = 0; attempt < ACL_VERIFY_ATTEMPTS; attempt += 1) {
      let value
      try {
        value = await windowsAccessList(path, ensure)
      } catch {
        lastUnavailable = new AgentHostError('STATE_ROOT_PERMISSIONS_UNAVAILABLE', 'Windows could not verify the private Agent Host access list')
        if (attempt + 1 < ACL_VERIFY_ATTEMPTS) {
          await delay(aclVerifyBackoffMs(attempt))
          continue
        }
      ...
      if (value.status === 'error') {
        ...
        lastUnavailable = new AgentHostError('STATE_ROOT_PERMISSIONS_UNAVAILABLE', `Windows access-list verification failed (${reason}, line ${line})`)
        if (attempt + 1 < ACL_VERIFY_ATTEMPTS) {
          await delay(aclVerifyBackoffMs(attempt))
          continue
        }
