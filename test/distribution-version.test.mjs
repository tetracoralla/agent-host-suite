...
test('internal Beta distribution derives one Host, Manager, catalog, and provenance version identity', async (t) => {
  ...
  await execFileAsync(process.execPath, [fileURLToPath(writer), dmg, catalog, destination])
...
test('distribution check rejects an unrelated same-version release catalog before mounting', { skip: process.platform !== 'darwin' }, async (t) => {
  const { catalog, dmg, destination } = await fixture(t)
  await execFileAsync(process.execPath, [fileURLToPath(writer), dmg, catalog, destination])
  ...
  await assert.rejects(
    execFileAsync('/bin/zsh', [fileURLToPath(distributionChecker), 'internal-beta', dmg, destination], {
...
test('internal Beta distribution refuses a release version that differs from the Host and Manager identity', async (t) => {
  ...
  await assert.rejects(
    execFileAsync(process.execPath, [fileURLToPath(writer), dmg, catalog, destination]),
...
test('macOS app packaging refuses a stale catalog before starting a Swift build', { skip: process.platform !== 'darwin' }, async (t) => {
  const { catalog } = await fixture(t, '0.1.0')
  const catalogRoot = join(catalog, '..')
  await assert.rejects(
    execFileAsync('/bin/zsh', [fileURLToPath(appPackager), 'debug'], {
