...
test('metadata observation records semantic identity and costs without input or result content', async () => {
  ...
  try {
    ...
    assert.equal(observation.schemaVersion, 'openadam.direct-execution-observation.v0.2')
    assert.equal(observation.purpose, 'unspecified')
    assert.deepEqual(observation.outcome, { status: 'not-reported', value: null })
