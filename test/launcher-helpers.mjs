...
export function runSkillLauncher(launcher, args, { cwd } = {}) {
  if (process.platform !== 'win32') return execFileSync(launcher, args, { encoding: 'utf8', cwd })
  ...
  return execFileSync(process.env.ComSpec ?? 'cmd.exe', ['/d', '/s', '/c', `""${launcher}" ${args.join(' ')}"`], {
    encoding: 'utf8', windowsVerbatimArguments: true, cwd,
  })
}
